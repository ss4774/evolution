...
def generate_program_plasmids(N_inputs, N_layers = 1, N_terms = 3, gates = ["AND"], full_connectivity=True): # generate program using only YES and NOT gates; 
    if type(N_terms) == int:
        N_terms = [N_terms]
        N_terms *= N_layers
...
def generate_program_plasmids_minterms(N_inputs): # generate all minterms for a given number of inputs    
    plasmids = {}
    plasmids_ids = []
    ...
    for i, inputs in enumerate(ins2):
        ...
        output = 'y'
        plasmid = generate_plasmid(inputs, output, models.AND00)
        plasmids[label] = plasmid
        plasmids_ids.append(label)

        label = f'P0_{i}_AND01'
        output = 'y'
        plasmid = generate_plasmid(inputs, output, models.AND01)
        plasmids[label] = plasmid
        plasmids_ids.append(label)

        label = f'P0_{i}_AND10'
        output = 'y'
        plasmid = generate_plasmid(inputs, output, models.AND10)
        plasmids[label] = plasmid
        plasmids_ids.append(label)

        label = f'P0_{i}_AND11'
        output = 'y'
        plasmid = generate_plasmid(inputs, output, models.AND11)
        plasmids[label] = plasmid
        plasmids_ids.append(label)
...
def generate_cells_minterms(N, N_inputs):
    plasmids, _ = generate_program_plasmids_minterms(N_inputs)  
    population = np.zeros((N,N), dtype=object)
